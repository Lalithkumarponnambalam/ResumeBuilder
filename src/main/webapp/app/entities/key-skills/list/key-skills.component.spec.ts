import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { of } from 'rxjs';

import { KeySkillsService } from '../service/key-skills.service';

import { KeySkillsComponent } from './key-skills.component';

describe('KeySkills Management Component', () => {
  let comp: KeySkillsComponent;
  let fixture: ComponentFixture<KeySkillsComponent>;
  let service: KeySkillsService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      declarations: [KeySkillsComponent],
    })
      .overrideTemplate(KeySkillsComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(KeySkillsComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(KeySkillsService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.keySkills?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });
});
